Дела 
	1 - При выборе категории нужно чтобы скролл поднимался моментально в начало страницы
	2 - Верстка, исправить мобильную версию для всех страниц и для desktop
	3 - Сделать скелетоны для страниц, удалить стандартную загрузку.
	4 - Пофиксить баг страницы корзины, избранное, чтобы при первом заходе пользователя при пустой корзине, избранное, которые еще не были созданны в базе не было бага, что они не созданы.
	5 - Сделать таймаут для тоста чтобы он не спамил (возможно как то можно по другому реализовать, более локанично. Возможно сделать свитчер, который будет менять тост который был и показывать новый.)
	6 - Сделать тесты для бэка, для фронта(это долго, будем делать в конце)
	7 - Рефакторинг бэка, переместить в сервисы методы, которые используются в моделях.
	8 - Безопасность вызываемых методов в бэке, добавить декораторы для методов всех если у них их нет.
	9 - Проверить или исправить сихронизацию ui пользователя, чтобы отображать актуальные данные о количестве товара и тд.(Возможно какие то микросервисы или веб сокеты, думать надо)
	10 - Деплой проекта.
